...
$(window).on('resize',Foundation.util.throttle(function(){ww=$(window).width();wh=$(window).height();isMobile=Foundation.MediaQuery.current==='small';isTablet=Foundation.MediaQuery.current==='medium';if(isMobile){$('.schedule .day-calendar').slideUp(0)}else{$('.schedule .day-calendar').slideDown(0)}},300));// init
$(document).ready(function(){$(window).resize();scrollTick()});// mobile size calendar fold
$('.schedule').on('click','.date-head',function(){if(isMobile){var $it=$(this).closest('.schedule').find('.day-calendar');$('.schedule .date-head').removeClass('open');$(this).addClass('open');$('.schedule .day-calendar').not($it).slideUp();$it.slideDown()}});// scroll tick
